...
class DepositDeviceForm(wx.Frame):
    ...
    def on_check_details(self, event):
        ...
        if device_id:
            # Check if Device ID is present in "Device_Issued_log.csv"
            issued_log_path = "Device_Issued_log.csv"
    ...
    def on_deposit_device(self, event):
        ...
        if device_id and user_id and device_type:
            ...
            deposit_log_path = "Device_Deposit_log.csv"
...
class IssueDeviceForm(wx.Frame):
    ...
    def on_issue_device(self, event):
        ...
        if user_id and device_id and device_type:
            # Check if Device ID is present in "Device_Registered_log.csv"
            registered_log_path = "Device_Registered_log.csv"
            issued_log_path = "Device_Issued_log.csv"
            ...
            # Show success message
            wx.MessageBox(f"Device Issued at time {issue_time}!", "Success", wx.OK | wx.ICON_INFORMATION)

            self.Close()
        else:
            wx.MessageBox("Please enter User ID, Device ID, and select Device Type!", "Issue Device Error", wx.OK | wx.ICON_ERROR)
...
class RegisterDeviceForm(wx.Frame):
    ...
    def on_register_device(self, event):
        ...
        if device_id and device_type:
            ...
            log_file_path = "Device_Registered_log.csv"
            try:
            ...
            duplicate_entry = device_log[(device_log['Device ID'] == device_id) & (device_log['Device Type'] == device_type)]
...
class LoginPage(wx.Frame):
    def __init__(self, *args, **kw):
        ...
        self.lobby_choices = ["", "Soccer / Association Football", "Cricket", "Field Hockey", "Tennis", "Volleyball", "Table Tennis", "Baseball", "Golf"]
...
class HomePage(wx.Frame):
    ...
    #         RegisterDeviceForm(self, lobby=self.lobby_label.GetLabel().replace("Lobby: ", "")).Show()
    #     else:
    #         wx.MessageBox(f"Button clicked: {button_label}", "Button Clicked", wx.OK | wx.ICON_INFORMATION)
    ...
    def on_button_click(self, event):
        ...
        if button_label == "Register New Device":
            ...
            IssueDeviceForm(self, lobby=self.lobby_label.GetLabel().replace("Lobby: ", "")).Show()
        elif button_label == "Deposit Device":
            DepositDeviceForm(self, lobby=self.lobby_label.GetLabel().replace("Lobby: ", "")).Show()
        else:
            wx.MessageBox(f"Button clicked: {button_label}", "Button Clicked", wx.OK | wx.ICON_INFORMATION)
